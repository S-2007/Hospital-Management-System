...
typedef struct {
    ...
    char type[30];   // consultation, follow-up, emergency
} Appointment;
...
typedef struct {
    ...
    char dosage[30];
    int duration; // days
    char frequency[20]; // once, twice, thrice daily
...
typedef struct {
    ...
    int quantity;
    float unitPrice;
    char category[30]; // medicine, equipment, supplies
    char expiryDate[12];
    int minThreshold;
} InventoryItem;
...
typedef struct {
    ...
    char startTime[6];
    char endTime[6];
    char shift[20]; // morning, afternoon, night
    int isAvailable;
} StaffSchedule;
